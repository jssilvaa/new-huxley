#!/usr/bin/env python3
"""Convert `perf report --stdio` text output into a small LaTeX table.

Typical input is `perf.report.full.txt` generated by `make_graphs.sh`.

Example:
  python3 scripts/profiling/perf_report_to_latex.py \
    profiles/perf/s1_coldstart_*/perf.report.full.txt --top 5 > top5.tex
"""
...
def _latex_escape(text: str) -> str:
    # Minimal, practical LaTeX escaping for table cells.
...
def _split_columns(line: str) -> Optional[PerfRow]:
    ...
    # `perf report --stdio` uses fixed-width columns separated by 2+ spaces.
